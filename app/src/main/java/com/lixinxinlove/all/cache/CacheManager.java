package com.lixinxinlove.all.cache;
...
public final class CacheManager {
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(App.Companion.getInstance(), CACHE_DIR);
        if (!diskCacheDir.exists()) {
            boolean b = diskCacheDir.mkdirs();
            Log.e("CacheManager", "!diskCacheDir.exists() --- diskCacheDir.mkdirs()=" + b);
        }
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(App.Companion.getInstance()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
                Log.e("CacheManager", "mDiskLruCache created");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
